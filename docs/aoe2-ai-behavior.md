...
**A THIRD axis, outside the SNs (corrected 2026-07-27):** the engine also scales the AI's **unit-training and research TIME** — Easiest **200%**, Easy **133%**, Moderate and above **100%**. So an easier AI researches the *same full tech list*, just slower; DE never caps WHICH technologies it may take. (Sources: [Fandom AI page](https://ageofempires.fandom.com/wiki/Artificial_intelligence), [AoEZone](https://aoezone.net/threads/ai-difficulties.106021/).) Ours diverges: `maxTech` 2/7/14 is a hard cap on tech COUNT, so our easy AI can never reach a third technology however long the game runs, where DE's eventually gets everything.
...
| Difficulty knob split | DE scales attack group size, reaction, AND unit/research TIME (Easiest 200% / Easy 133% / Moderate+ 100%); economy, aging, build order and town size are identical | **aligned 2026-07-27**: 20 knobs had easy==hard with MEDIUM the outlier — queueLimit, houseBuffer, buildersPerBuilding, maxBarracks, barracksVil, waveCooldown, attackTick, armyReserve, ramWoodReserve, militaryFoodReserve, wallVils, attackAdvantage, maxTradeCarts, marketVil, ecoRatios, farmShare, targetFarms, allyJoin*, ageSurgeFactor. Easy and hard were retuned together (easy = hard's build behaviour throttled by RATES) and medium kept its older values, leaving the middle rung economically WEAKER than easy and *less* aggressive on timing (attackTick 6000 vs 3600). Medium now takes the easy/hard value on all 20; only the rate ladder scales — armyPerVil 0.3/0.6/0.9, commitPercent 35/56/75, waveCap 8/12/24, attackSize 3/4/5, decisionInterval 300/180/120 (reaction), plus the deliberate hard buffs maxVils/maxTech. **maxTech is a known divergence**: DE caps no tech COUNT, it slows the RATE, so our easy AI is permanently stuck at 2 technologies while DE's merely gets there later — a research-time multiplier would be the faithful mechanism. Ladder re-verified 1v1 x30 seeds on medium: medium beats easy 94% of decided, hard beats medium 91% — monotonic. Honest caveat: medium already won 86% pre-alignment, so this is a COHERENCE fix, not a rescued ladder | ✅ |
...
| Behavior | AoE2 DE | Ours | Verdict |
|---|---|---|---|
| Wall SHAPE | `town-wall-pattern`, script-driven; DE walls situationally and often partially | **barrier-anchored rectangle** (2026-07): each side slides outward within a band to the offset needing the fewest BUILT tiles, so the ring hugs forest/water/ore instead of running a square across open ground (`terrainBarrier` tiles seal for free). Measured: ~1.7-2x the enclosed area at equal-or-lower build cost, and rings now COMPLETE (99-100% vs 16-56% for the old square). Where no cover exists it degrades to the old square, so it never costs more. Rejected first: a uniform DE-sized radius (r9) — it plateaus at 44% built and never seals, and our whole defensive model (gate/egress/stone-upgrade) keys off `aiBaseSealed`, so partial rings mean 33 dead teams vs 9. DE can wall partially because DE walls *situationally*; we cannot copy the look without the mechanism | ✅ |
...
| Resource floors | `sn-minimum-<res>` | ONE deficit-driven market rule (2026-07 unification): `MARKET_FLOOR` {food 100, wood 80, gold 150} / `MARKET_SURPLUS` {stone 200, wood 500, food 400} — each decision tick fix the worst floor breach (buy with gold down to a 100 cushion; gold-poor or gold-breached → sell the first surplus, stone first), else a gold-rich wood top-up. Wealth locked in the wrong commodity converts toward whatever starves in ≤2 hops (surplus→gold→need). A starving-with-gold OR war-state AI may build the market NEED-based; barracks rebuild fund (175w) outranks towers/market/new farms (`aiBarracksFundClear`) | ✅ minimal analog |
