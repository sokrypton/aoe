...
if (typeof window !== 'undefined' && window.location && window.location.search.includes('testwall')) {
  setMapSize('medium');
  window.fogDisabled = true;
  restartGame('standard');

  let px = STARTS[0].x, py = STARTS[0].y;
  
  // Clear the area
  for (let y = py - 8; y <= py + 8; y++) {
    for (let x = px - 8; x <= px + 8; x++) {
      if (map[y] && map[y][x]) {
        map[y][x].t = TERRAIN.GRASS;
      }
    }
  }

  // Clear existing entities in this area
  entities = entities.filter(en => Math.abs(en.x - px) > 8 || Math.abs(en.y - py) > 8);

  // 1. Spawn Corner Towers
  createBuilding('TOWER', px - 4, py - 4, 1).complete = true;
  createBuilding('TOWER', px + 4, py - 4, 1).complete = true;
  createBuilding('TOWER', px - 4, py + 4, 1).complete = true;
  createBuilding('TOWER', px + 4, py + 4, 1).complete = true;

  // 2. Top Wall (E-W, from px-3 to px+3 at py-4)
  createBuilding('WALL', px - 3, py - 4, 1).complete = true;
  createBuilding('WALL', px - 2, py - 4, 1).complete = true;
  let gTop = createBuilding('GATE', px - 1, py - 4, 1, 2, 1); gTop.complete = true; // Closed E-W
  createBuilding('WALL', px + 1, py - 4, 1).complete = true;
  createBuilding('WALL', px + 2, py - 4, 1).complete = true;
  createBuilding('WALL', px + 3, py - 4, 1).complete = true;

  // 3. Bottom Wall (E-W, from px-3 to px+3 at py+4)
  createBuilding('WALL', px - 3, py + 4, 1).complete = true;
  createBuilding('WALL', px - 2, py + 4, 1).complete = true;
  let gBot = createBuilding('GATE', px - 1, py + 4, 1, 2, 1); gBot.complete = true;
  gBot.gateProgress = 1.0; // Open E-W
  createBuilding('WALL', px + 1, py + 4, 1).complete = true;
  createBuilding('WALL', px + 2, py + 4, 1).complete = true;
  createBuilding('WALL', px + 3, py + 4, 1).complete = true;

  // 4. Left Wall (N-S, from py-3 to py+3 at px-4)
  createBuilding('WALL', px - 4, py - 3, 1).complete = true;
  createBuilding('WALL', px - 4, py - 2, 1).complete = true;
  let gLeft = createBuilding('GATE', px - 4, py - 1, 1, 1, 2); gLeft.complete = true; // Closed N-S
  createBuilding('WALL', px - 4, py + 1, 1).complete = true;
  createBuilding('WALL', px - 4, py + 2, 1).complete = true;
  createBuilding('WALL', px - 4, py + 3, 1).complete = true;

  // 5. Right Wall (N-S, from py-3 to py+3 at px+4)
  createBuilding('WALL', px + 4, py - 3, 1).complete = true;
  createBuilding('WALL', px + 4, py - 2, 1).complete = true;
  let gRight = createBuilding('GATE', px + 4, py - 1, 1, 1, 2); gRight.complete = true;
  gRight.gateProgress = 1.0; // Open N-S
  createBuilding('WALL', px + 4, py + 1, 1).complete = true;
  createBuilding('WALL', px + 4, py + 2, 1).complete = true;
  createBuilding('WALL', px + 4, py + 3, 1).complete = true;
}
