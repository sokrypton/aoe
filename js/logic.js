...
function resolveStalledAttack(u, tgt){
  ...
  if (w && w.id !== stalledId && !sameSide(w.team, u.team)) {
    ...
    u.unreachId = stalledId; u.unreachUntil = tick + (tgt.type === 'building' ? T30(900) : UNREACH_UNIT_TICKS);
  } else {
    ...
    u.unreachId = stalledId; u.unreachUntil = tick + (tgt.type === 'building' ? T30(300) : UNREACH_UNIT_TICKS);
  }
    ...
    if (range > 0) {
      ...
      if (d > range) {
        ...
        if(!e.explicitAttack && e.path.length > d*2+10){
          e.target=null;
          e.unreachId=t.id; e.unreachUntil=tick+UNREACH_UNIT_TICKS;
