...
// rotate around the blit. Cached at ceil(ZOOM) resolution so the raster has ≥1
// texel per on-screen pixel (crisp at any zoom); only ~5 size × 2 darken × a
// couple zoom buckets ever exist, so no eviction. Anchor (ax,ay) is where the
// tree's (sx,cy) lands in the canvas. Render-only — no determinism impact.
...
function drawTreeEntity(x,y){
  ...
  if(t.res > 60 || isFalling){
    ...
    // as a rotate about (sx,cy). Cache at ceil(ZOOM) so it stays crisp.
    let idx = (x * 17 + y * 23) % 5;
