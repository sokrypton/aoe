...
const SCENE = (frames, fade) => `(() => {
  window.__occluderFade = ${fade ? 'true' : 'false'};
  // ---- dense stage ----
  ...
  render(); // one more so the *_active/byTeam counters reflect a real frame
  let occluded=0; try{ _bsilByTeam.forEach(a=>occluded+=a.length); }catch(e){}
  let activeOcc=0; try{ activeOcc=_bsilActive.length; }catch(e){}

  return {
    zoom: ZOOM, frames: N,
    scene: { units: nu, buildings: nb, trees: 256, selected: selected.length, occludedUnits: occluded, activeOccluders: activeOcc },
...
(async () => {
  ...
  try {
    ...
    for (const z of zooms) {
      ...
      r.mode = fade ? 'FADE (transparent buildings)' : 'OUTLINE';
